...
node unitejs/cli/bin/unite configure --packageName=vu-bfy-ts-jas-pro --title="Vue Browserify TypeScript" --sourceLanguage=TypeScript --moduleType=CommonJS --bundler=Browserify --unitTestRunner=Jest --unitTestFramework=Jasmine --unitTestEngine=JSDom --e2eTestRunner=Protractor --e2eTestFramework=Jasmine --linter=TSLint --license=MIT --packageManager=Npm --cssPre=Css --cssPost=None --appFramework=Vue --ides=vscode --disableVersionCheck --outputDirectory=./apps/vu-bfy-ts-jas-pro
